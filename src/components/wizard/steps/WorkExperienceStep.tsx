...
interface WorkItem {
  ...
  to: string;       // YYYY-MM
  projects: string; // optional text
  documents: (File | string)[] | null; // File while editing; URLs saved; or null
}
...
interface WorkExperienceStepProps {
  ...
  initialWorkExperience?: WorkItem[] | null;
}
...
const WorkExperienceStep = ({
  ...
  const emptyRow: WorkItem = useMemo(
    () => ({ company: "", role: "", location: "", from: "", to: "", projects: "", documents: null }),
    []
  );

  const [experiences, setExperiences] = useState<WorkItem[]>([emptyRow]);
  const [saving, setSaving] = useState(false);
  ...
  const addExperience = () => setExperiences((prev) => [...prev, { ...emptyRow }]);
  ...
  const toDbPayload = (items: WorkItem[], uploadedPerExp: (string | null)[][]) =>
    items.map((it, i) => {
      // keep existing string URLs + add new uploads
  ...
  useEffect(() => {
    ...
    const hydrate = async () => {
      try {
        if (initialWorkExperience && initialWorkExperience.length) {
          if (!cancelled) {
        ...
        const ui = fromDbPayload(data?.work_experience ?? []);
        if (!cancelled) {
          setExperiences(ui.length ? ui : [emptyRow]);
          setHydrating(false);
        }
      } catch (e) {
        console.error(e);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Upload files per experience (keeps existing string URLs)
      const uploadedPerExp: (string | null)[][] = [];
      for (let i = 0; i < experiences.length; i++) {
        uploadedPerExp.push(await uploadDocsIfNeeded(client_id, experiences[i], i));
      }
      ...
      const { error: upsertErr } = await supabase
        .from("client_profiles")
        .upsert(
          { client_id, work_experience: payload, progress_percent: 56 }, // adjust as needed
          { onConflict: "client_id" }
        );

      if (upsertErr) {
        console.error("Upsert error:", upsertErr);
        alert("Failed to save work experience. Check console.");
        setSaving(false);
        return;
      }
    ...
    >
      {hydrating ? (
        ...
        <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => handleFilesChange(index, e.target.files)}
                    />
                    {/* Show existing uploaded docs (string URLs) */}
                    {Array.isArray(exp.documents) && exp.documents.some((d) => typeof d === "string") && (
                      <ul className="mt-2 space-y-1">
                        {(exp.documents as (File | string)[])
          ...
          <Button type="button" variant="outline" onClick={addExperience} className="w-full mt-6">
            <Plus className="w-4 h-4 mr-2" />
            Add Another Experience
          </Button>
        </>
