...
const CandidateOverview = () => {
  ...
  const matchBreakdown = useMemo(() => {
    return [
      { label: "Education", score: viewModel?.sectionDetails.education.items.length ?? 0, maxScore: 5, color: "bg-primary" },
      { label: "Certifications", score: viewModel?.sectionDetails.certifications.items.length ?? 0, maxScore: 5, color: "bg-info" },
      { label: "Internships", score: viewModel?.sectionDetails.internships.items.length ?? 0, maxScore: 5, color: "bg-warning" },
      { label: "Work", score: viewModel?.sectionDetails.work.items.length ?? 0, maxScore: 5, color: "bg-success" },
      { label: "Projects", score: viewModel?.sectionDetails.projects.items.length ?? 0, maxScore: 5, color: "bg-primary" },
      { label: "Skills", score: (viewModel?.sectionDetails.tech.items[1]?.chips?.length ?? 0) > 0 ? 3 : 0, maxScore: 3, color: "bg-info" },
      { label: "Resume", score: (viewModel?.sectionDetails.social.items[0]?.files?.length ?? 0) > 0 ? 2 : 0, maxScore: 2, color: "bg-success" },
      { label: "Assessments",   score: 0, maxScore: 7, color: "bg-warning" },
    ];
  }, [viewModel]);
